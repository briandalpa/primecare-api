...
describe('errorMiddleware', () => {
  ...
  it('should handle ResponseError with 400 status', () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ status: 'error', message: 'Validation error' });
  });
  ...
  it('should handle ResponseError with 422 status', () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ status: 'error', message: 'No outlet available in your area' });
  });
  ...
  it('should handle ZodError with 400 status', () => {
    ...
    expect(res.status).toHaveBeenCalledWith(400);
    // Issues are flattened to a single message string; never expose ZodError internals.
    expect(res.json).toHaveBeenCalledWith({
      status:  'error',
      message: 'name: Expected string',
    });
  });
  ...
  it('should handle unknown error type with 500 status', () => {
    ...
    errorMiddleware(error as unknown as Error, req as Request, res as Response, next);
    expect(res.status).toHaveBeenCalledWith(500);
    expect(res.json).toHaveBeenCalledWith({ status: 'error', message: 'Internal server error' });
  });
});
