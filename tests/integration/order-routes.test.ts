...
jest.mock('@/application/database', () => {
  ...
  return {
    prisma: {
      ...
      $transaction: jest
        .fn()
        .mockImplementation(async (cb: Function) => cb({ order: txOrderMock })),
    },
  };
});
...
describe('GET /api/v1/orders', () => {
  ...
  it('returns 200 with paginated envelope on success', async () => {
    authenticatedAsCustomer();
    orderMock.findMany.mockResolvedValue([mockOrderRow] as never);
    orderMock.count.mockResolvedValue(1 as never);
  ...
  it('returns 200 when excludeCompleted=true is passed', async () => {
    authenticatedAsCustomer();
    orderMock.findMany.mockResolvedValue([] as never);
    orderMock.count.mockResolvedValue(0 as never);

    const res = await request(app).get('/api/v1/orders?excludeCompleted=true');

    expect(res.status).toBe(200);
    expect(res.body.data).toHaveLength(0);
  });

  it('returns 400 when status query param is an invalid enum value', async () => {
    authenticatedAsCustomer();
